function [data_deconv,event,HRF,adjust_global,PARA] = wgr_deconv_canonhrf_par_mod(data,thr,event_lag_max,TR,p_m,nworker)
...
[N, nvar] = size(data);
fprintf('Starting deconvolution (th = %.2f, lagmax = %.2f, mode=%i)\n',thr,event_lag_max,p_m);
fprintf('...data dimension: %i signals with length %i\n',nvar,N);

fprintf('...number of NaN''s in input: %i\n',nnz(isnan(data)));
...
if nworker>1
    ...
    if nargout>1
        ...
        parfor i=1:nvar
            data_deconv(:,i) = wgr_adjust_onset(data(:,i),even_new{i},event_lag_max,TR,p_m,T,N);
        end
    end
    fprintf(' done (%.2ss/timeserie)\n',toc/nvar);    
    
else
    
    printind = round(linspace(nvar/10,nvar,10));
    kk=1;
    warning off
    fprintf('...deconvolving');
    tic;
...
fprintf('...number of NaN''s in output: %i\n',nnz(isnan(data_deconv)));
...
param = zeros(3,event_lag_max+1);
Cov_E = nan(1,event_lag_max+1);
