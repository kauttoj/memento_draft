...
%% Initialisation %%
%%%%%%%%%%%%%%%%%%%%
function [count1,count2] = memento_rsa_ver5_looper(FOLDER,RADIUS)
...
for s=1:length(subj_averaged_volumes)
    ...
    for i=1:nConditions
        ...
        for j=1:nConditions
            if i==j
                a(i,j)=0;
            elseif strcmp(subj_averaged_volumes{s}(i).type,'BW') && strcmp(subj_averaged_volumes{s}(j).type,'BW')
                a(i,j)=0;
            end
        end
    end
    all_models{s}(1).name = 'bw-model';
...
userOptions.seed = 9846;
userOptions.analysisName = 'mementoRSA';
userOptions.random_iterations = 500000;
...
% prepare the rMaps:
LOWER_LIMIT = max(10,ceil(searchlightOptions.N_searchlight*0.40));
supermask = mask;
...
for subjectI = 1:Nsubjects
    ...
    supermask=supermask.*(ns>=LOWER_LIMIT);
...
supraThreshMarked_t=extentThreshold(supraThreshMarked_t,20);
supraThreshMarked_sr=extentThreshold(supraThreshMarked_sr,20);

meanmap_t = extentThreshold(meanmap_t,20);
meanmap_sr = extentThreshold(meanmap_sr,20);

meanmap_t_p0001 = extentThreshold(meanmap_t_p0001,40);
meanmap_sr_p0001 = extentThreshold(meanmap_sr_p0001,40);
...
count1 = nnz(meanmap_t);
count2 = nnz(meanmap_t_p0001);
