function compute_roi_correlations_stage2_deconv(DELAYS)
...
GOOD_IDs = [1,2,3,4,5,6,9,10,11,12,13,14];
TIMING_data = TIMING_data(GOOD_IDs);
...
for l=1:length(DELAYS)
    ...
    for i=1:length(TIMING_data)
        ...
        [~,best_ind]=min(abs(TIMING_data(i).SECOND_time(1)+DELAY - volume_times));        
        if abs(TIMING_data(i).SECOND_time(1)+DELAY - volume_times(best_ind-1))<abs(TIMING_data(i).SECOND_time(1)+DELAY - volume_times(best_ind+1))
            second_best_ind=best_ind-1;
        else
            second_best_ind=best_ind+1;
        end
        
        fmri_volumes = sort([best_ind,second_best_ind]);                
...
save('volume_selection_data.mat','averaged_volumes_all','volume_times','DELAYS','S','TIMING_data','-v7.3');
...
for subj_ind = 1:length(S);
    ...
    clear data;
    load([subj,'_roi_data_deconvolved.mat']);
    ...
    for l=1:length(averaged_volumes_all)
        ...
        for i = 1:length(averaged_volumes)
            ...
            if max(ind)-min(ind) > 5 || any( diff(ind)~=1)
                max(ind),min(ind)
