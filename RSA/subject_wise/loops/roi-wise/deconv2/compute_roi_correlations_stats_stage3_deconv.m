function compute_roi_correlations_stats_stage3_deconv(ITERATIONS,nworker)
...
volume_times = (0:TR:(36*60)) + first_volume_offset;
...
for i=1:length(COLOR_SEGMENTS)
    ...
    [~,low]=min(abs(COLOR_SEGMENTS(i).ORIGINAL_time(1) - volume_times));    
    [~,high]=min(abs(COLOR_SEGMENTS(i).ORIGINAL_time(2) - volume_times));
    
    fmri_volumes = low:high;
    fmri_volumes(end-2:end)=[];
    fmri_volumes(1:3)=[];
...
parfor subj_ind = 1:length(S);
    ...
    cd(HOME);
    data = load_data([subj,'_roi_data_deconvolved.mat']);
    ...
    for iter = 1:ITERATIONS
        ...
        for j=1:N_rois
            ...
            subj_nullvals(j,iter)=mean(cormat(indd));
        end                                
    ...
    save_data([subj,'_nullvals_deconvolved.mat'],subj_nullvals);

end

subj_nullvals_deconvolved=[];

all_nullvals_deconvolved = zeros(N_rois,ITERATIONS);
for subj_ind = 1:length(S);
    
    subj = S{subj_ind};
    load([subj,'_nullvals_deconvolved.mat']);
    all_nullvals_deconvolved = all_nullvals_deconvolved+subj_nullvals_deconvolved;
    
end
all_nullvals_deconvolved=all_nullvals_deconvolved/length(S);

cd(HOME);
save('combined_nullvals_deconvolved','nulldata','all_nullvals_deconvolved','S','my_rois','my_masks','searchlightRad_mm','ITERATIONS','N_workers','-v7.3');

diary off

fprintf('\n--- ALL DONE!! ----\n')

function save_data(filename,subj_nullvals_deconvolved)

save(filename,'subj_nullvals_deconvolved','-v7.3');
