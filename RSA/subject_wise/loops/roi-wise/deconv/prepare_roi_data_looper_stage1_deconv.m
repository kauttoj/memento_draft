...
parfor subj_ind = 1:length(S)
    ...
    for ses=1:3        
        ...
        for k=1:length(data)
            ...
            data{k} = wgr_deconv_canonhrf_par(data{k},deconvolve_thr,deconvolve_event_lag_max,TR,1);         
        end                
        fprintf('done!\n');
        all_roi_data{ses}=data;        
    end    
    cd(HOME);
    res = save_data(all_roi_data,[subj,'_roi_data_deconvolved.mat']);
    if res == 0
        error('Failed to save!')            
    end    
    fprintf('Z-scoring data (second standardization)... ');
    for ses=1:3   
        for k=1:length(all_roi_data{ses})
            all_roi_data{ses}{k}=zscore(all_roi_data{ses}{k});
        end
    end
    fprintf('done!\n');
    res = save_data(all_roi_data,[subj,'_roi_data_deconvolved_zscored.mat']);
    if res == 0
        error('Failed to save!')            
    end    
end
