...
correlations = zeros(length(S),N_rois,length(DELAYS));
...
correlations = atanh(correlations);
df = max(length(S) - 1,0);
samplesize = length(S);
for j=1:N_rois
...
t_th = fzero(@(x) tcdf(-x,df)-0.0001,1);
good_rois = find((sum(tvals>t_th,2))>3);

good_rois=[];
for j=1:N_rois
    if nnz(mean_correlations(DELAYS<7,j)>threshold1(j))>1
        good_rois(end+1)=j;
    end
...
for i=unique(T2')
    ...
    for h=j
        dist(h)=corr2(selected_data(h,:),cmeans(i,:));
    end
end
T2=T2(w);
silval=silval(w);
...
H_BrainNet = BrainNet_MapCfg_custom(my_rois,a,T2,[],[],[]);
